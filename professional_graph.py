...
def classify_node(name: str) -> str:
    """Assign domain label based on node name/path."""
    n = name.lower()
    if n.startswith("#"):
        return "tag"
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .spinner {
    ...
    border: 3px solid rgba(0,206,201,0.15);
...
document.getElementById('stats').textContent =
  `${nodeCount} nodes  ·  ${edgeCount} links  ·  ${domainCount} domains`;

// ── Domain colors for legend ──────────────────────────────────────────────────
...
const link = g.append('g').attr('class','links')
  ...
  .attr('stroke-width', d => Math.sqrt(d.weight || 1) * 0.6)
...
const node = g.append('g').attr('class','nodes')
  ...
  .attr('cursor', 'pointer')
  .attr('opacity', d => (d.id.startsWith('#') && !activeDomains.has('tag')) ? 0.25 : 1);

// Labels for big nodes
const label = g.append('g').attr('class','labels')
  .selectAll('text')
  .data(nodes.filter(n => n.degree > 8 && !n.id.startsWith('#')))
  .join('text')
  .text(d => d.label)
...
node.on('mousemove', (event, d) => {
  ...
  ttMeta.innerHTML    = `<strong>Degree</strong> ${d.degree} &nbsp;·&nbsp; <strong>Community</strong> ${d.community}`;
...
searchEl.addEventListener('input', () => {
  ...
  node.attr('opacity', d => {
    if (!q) return 1;
    return d.label.toLowerCase().includes(q) || d.domain.toLowerCase().includes(q) ? 1 : 0.08;
  });
...
function updateVisibility() {
  node.attr('opacity', d => {
    if (d.id.startsWith('#')) return tagsVisible ? 0.85 : 0;
    if (!activeDomains.has(d.domain)) return 0.06;
    if (searchEl.value) {
      const q = searchEl.value.toLowerCase();
      return d.label.toLowerCase().includes(q) || d.domain.toLowerCase().includes(q) ? 1 : 0.08;
    }
